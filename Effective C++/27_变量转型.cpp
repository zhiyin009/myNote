...
int main() {
  ...
  int *pi =
      reinterpret_cast<int *>(pb);  // 实际动作取决于编译器，意味着它不可移植。
...
/*
    要点：
    1.转型操作：
    const_cast<T>(expression)：通常用来将对象的常量性转除（cast away the
   constness）。 dynamic_cast<T>(expression)：主要用来执行“安全向下转型”（safe
   downcasting）。 reinterpret_cast<T>(expression)：意图执行低级转型。
    ...
    4.dynamic转型效率较低，可以使用容器存储指向derived
   class对象的指针或者使用virtual函数。
