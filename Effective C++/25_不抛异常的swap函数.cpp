...
// 针对“T是Widget”而设计的。换句话说一般性的swap
// template施行于Widgets身上就会启用这个版本。
...
// 然而假设Widget和WidgetImpl都是class
// template而非class，将WidgetImpl内的数据类型 加以参数化：
...
// 这样做，我们将试图偏特化一个function template，但c++只允许对class
// template偏特化，无法通过编译, 试图改为添加重载版本
namespace std {
template <typename T>
...
// 如何在他人调用swap时能够取得我们提供的高效template特定版本。我们可以声明一个non-member
// swap让 它调用member swap，但不再将non-member
// swap声明为std::swap的特化版本或重载版本。假设Widget的
...
/*
    **成员版swap绝不可抛出异常**
    这一约束只施行于成员版！不可施行于非成员版，因为swap缺省版本是以copy构造函数和copy
   assignment操作符
...
/*
    ...
    2.如果提供一个member swap，也应该提供一个non-member
   swap来调用前者，对于classes，请特化std::swap。
